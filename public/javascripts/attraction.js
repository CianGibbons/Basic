...
const jsonAttractionAdd = async (data) => {
  try {
    ...
    const url = `http://danu7.it.nuigalway.ie:8611/attractions`;
    const res = await axios({//using axios to send the request
    ...
    document.getElementById("results").innerHTML = JSON.stringify(err.response.data.message);
  }
};
...
const getAddAttraction = async (
  ...
  try {
    ...
    const res = await axios({
      ...
      data: {//insert the data to be posted
        name,
        city,
        wikiSlug,
        aType,
...
if (guiForm) { // if the GUI Form for adding attractions exists
...
if (jsonForm) {// if the form exists
...
const delAttr = async (id) => {
  try {//this method is to delete an attraction document from the collection
    //set the endpoint url
    const url = `http://danu7.it.nuigalway.ie:8611/attractions/${id}`;
    await axios({//no need to store the response because with a status 204 of deleted there is none
...
const getFilteredData = async (city, aType, page, limit) => {
  try {
    ...
    let url = "http://danu7.it.nuigalway.ie:8611/attractions/";
    url += `?page=${page}&limit=${limit}`;
    if (city) url += `&${city}`;
    if (aType) url += `&${aType}`;
    const res = await axios({// sends the request with axios
    ...
    if (res.data.status === "success") {
      ...
      attractions.forEach((attraction) => {//for each attraction append the following to the innerHTML of the results box
...
if (filterForm) {//if the filter form exists
...
const patchAttr = async (
  ...
  try {//set the url for this endpoint
    const url = `http://danu7.it.nuigalway.ie:8611/attractions/${aID}`;
    const res = await axios({//send the PATCH request with axios
      method: "PATCH",//set the method to patch
      url,//set the url to the one defined above
      data: {// set the data to be the data passed into our function and ensure it follows the structure of the model ie. type, coordinates,address, shortDescription, longDescription within locations
        name,
        city,
        wikiSlug,
        aType,
...
if (editForm) {
  ...
  editForm.addEventListener("submit", async (e) => {
    ...
    aID = aID.replace(`http://danu7.it.nuigalway.ie:8611/editAttraction/`, "");
