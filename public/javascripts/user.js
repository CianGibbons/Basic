const delUser = async (id) => {
  try {
    const url = `http://danu7.it.nuigalway.ie:8611/users/${id}`;
    await axios({// send axios request to delete the user
      method: "DELETE",
      url,
    });
...
const getUsers = async (sort, limit, page, username, email) => {
  try {
    let url = `http://danu7.it.nuigalway.ie:8611/users/`;
    ...
    if (res.data.status === "success") {
      ...
      users.forEach((thisuser) => {//for each user add the following into the innerHTML of the div with class results
...
if (userFilterForm) {// if the user filter form exists add event listener for the submission of the form
...
const patchUserInfo = async (
  ...
  try {
    ...
    if (photo) {//if photo exists
      ...
      res = await axios({//send request with axios
        method: "PATCH",//patch request
        url,
        data: {//send the data to be patched
          name,
          username,
          location,
          photo,
    ...
    } else {//send the request without the photo field
      res = await axios({
        method: "PATCH",
        url,
        data: {
...
if (userEditForm) {
  userEditForm.addEventListener("submit", async (e) => {
    ...
    id = id.replace("http://danu7.it.nuigalway.ie:8611/editUser/", "");
