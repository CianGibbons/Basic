...
const deleteUA = async (id) => {
  try {
    ...
    let url = `http://danu7.it.nuigalway.ie:8611/user/attractions?`;
    url += `attraction=${id}`;
    url += `&user=${userid}`;
    ...
    if (res.data.status === "success") {
      ...
      url = `http://danu7.it.nuigalway.ie:8611/user/attractions/${uaID}`;//use the user attraction id make the correct url endpoint to delete this user attraction
      await axios({//send delete request with axios
