...
if (locations.length != 0) {//if there are locations
  bounds = new mapboxgl.LngLatBounds();//initialise the bounds

  locations.forEach((loc) => {//for each location
    //create marker
    ...
    new mapboxgl.Marker({//creates the marker with the div we specified above
      element: el,
    ...
    map.on("mouseleave", locations, function () {//change the cursor back to normal when you stop hovering over the location marker
...
const getDistances = async (coordinates, units, cityId) => {
  try {
    ...
    const url = `http://danu7.it.nuigalway.ie:8611/attractions/distance/${coordinates[1]},${coordinates[0]}/unit/${units}?cityId=${cityId}`;
    ...
    if (res.data.status === "success") {
      ...
      attractions.forEach((attraction) => {
        ...
        document.getElementById( //get the attraction's distance display div by its unique id and set the innerHTML to the distance and set the display to be block rather than none
...
const addLocation = async (coordinates) => {
  ...
  new mapboxgl.Marker({//create the marker using the div and anchor to the bottom
    element: el,
    anchor: "bottom",
  })
    .setLngLat(coordinates)//set the coordinates of the marker
    .addTo(map);//add the marker to the map
  if (bounds) {// if bounds exists
    bounds.extend(coordinates);//extend the bounds to include the new marker
    map.fitBounds(bounds, {//fit the map to the bounds and give it some padding
      padding: {
        top: 20,
        bottom: 5,
        left: 20,
...
if (userLocation) {// if userLocation exists add event listener to listen for the submission of the form
